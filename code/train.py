...
parser.add_argument('--output_dir', type=str, default='train_results', help='Directory that stores all training logs '
                                                                            'and trained models')
...
def load_and_enqueue(sess, enqueue_op, pointgrid_ph, cat_label_ph, seg_label_ph):
    for epoch in range(1000 * TRAINING_EPOCHES):
        ...
        for loop in range(len(TRAINING_FILE_LIST)):
            mat_content = scipy.io.loadmat('../data/ShapeNet/train/' + \
                                           TRAINING_FILE_LIST[train_file_idx[loop]] + '.mat')
            ...
            sess.run(enqueue_op, feed_dict={pointgrid_ph: pointgrid, cat_label_ph: cat_label,
        ...
        with tf.device('/gpu:'+str(FLAGS.gpu)):
            ...
            queue = tf.FIFOQueue(capacity=20*batch_size, dtypes=[tf.float32, tf.float32, tf.float32],
                                 shapes=[[model.N, model.N, model.N, model.NUM_FEATURES],
                                                                 [model.NUM_CATEGORY],
                                                                 [model.N, model.N, model.N, model.K+1,
                                                                  model.NUM_SEG_PART]])
